...
y_train = pd.read_csv("Parameter Optimisation/1. Preprocessing/SMOTENC/y_train.csv").values.ravel() # flatten to 1d array
...
# learning_rates = np.logspace(-4, -1, num=10)  # 10 values between 10^-4 and 10^-1
learning_rates = [0.0001, 0.0002, 0.0005, 0.001, 0.002, 0.005, 0.01, 0.02, 0.05, 0.1]
print(learning_rates) # values to be tested
num_trials = len(learning_rates)
...
for i, lr in enumerate(learning_rates, start=1):
    print(f"Coarse Trial {i}/{num_trials}: Training with learning rate: {lr}")
...
results_df.to_csv("Parameter Optimisation/2. Learning Rate/lr_results.csv", index=False)

# Identify the best learning rate (based on highest F1-score)
best_idx = results_df['F1-Score'].idxmax() # get index of largest f1 score
best_lr_data = results_df.loc[best_idx] # save row of largest f1 score to new variable

# Save the best learning rate in a text file
with open("Parameter Optimisation/2. Learning Rate/best_learning_rate.txt", "w") as f:
    f.write(str(best_lr_data["Learning Rate"]))

print("\nBest Learning Rate and Associated Metrics:")
print(best_lr_data.to_string(index=True))

#%% Plotting Coarse Search Results

# Plot Training and Validation Loss
plt.figure(figsize=(8, 6))
plt.plot(learning_rates, results_df['Final Train Loss'], marker='o', label='Training Loss')
plt.plot(learning_rates, results_df['Final Val Loss'], marker='s', label='Validation Loss')
plt.xscale('log')
plt.xticks(learning_rates, [f'{lr:.0e}' for lr in learning_rates], rotation = 45)  # Set xticks to learning rates
...
plt.grid(True)
plt.tight_layout()
plt.savefig("Parameter Optimisation/2. Learning Rate/loss_vs_lr_plot.png", dpi=600)
plt.show()
